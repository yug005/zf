  ...
  return (
    <div className="relative h-24 w-24 flex-shrink-0">
      <svg viewBox="0 0 80 80" className="h-full w-full -rotate-90">
        <circle cx="40" cy="40" r="38" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="4" />
        <motion.circle
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-slate-500">score</span>
      </div>
    </div>
  );
}
...
function StatPill({ label, value, accent }: { label: string; value: string | number; accent: string }) {
  return (
    <motion.div
      variants={fadeUp}
      className="flex items-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.02] px-5 py-4 backdrop-blur-sm"
    >
  ...
  return (
    ...
    >
      ...
      <motion.div variants={stagger} className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        <motion.div
          variants={fadeUp}
          className="col-span-2 lg:col-span-1 flex items-center gap-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] px-5 py-4"
        >
      ...
      <motion.div variants={stagger}>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Service Status</h3>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="truncate text-sm font-bold text-white">{monitor.name}</span>
                      </div>
                      <p className="mt-1.5 truncate font-mono text-[10px] text-slate-500">{monitor.url}</p>
                    </div>
                    <div className="flex-shrink-0 rounded-lg border border-white/5 bg-white/[0.03] p-1.5">
                      <Icon className="h-3.5 w-3.5 text-slate-500" />
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-3 gap-2">
                    <div>
                      <p className="text-[9px] font-semibold uppercase tracking-wider text-slate-600">Uptime</p>
                      <p className="mt-0.5 text-xs font-bold text-slate-300">{formatPercentage(monitor.uptimePercentage)}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-semibold uppercase tracking-wider text-slate-600">Latency</p>
                      <p className="mt-0.5 text-xs font-bold text-slate-300">{formatResponseTime(monitor.avgResponseTimeMs)}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-semibold uppercase tracking-wider text-slate-600">Checked</p>
                      <p className="mt-0.5 text-xs font-bold text-slate-300">{formatRelativeTime(monitor.lastCheckedAt)}</p>
                    </div>
                  </div>
                </Link>
              </motion.div>
      ...
      {insights.attentionMonitors.length > 0 && (
        <motion.div variants={fadeUp} className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-xs font-bold uppercase tracking-[0.15em] text-rose-400">Requires Attention</h3>
          </div>
          <div className="space-y-2">
            {insights.attentionMonitors.slice(0, 4).map((m) => (
              <Link
                key={m.id}
                to={`/monitors/${m.id}`}
                className="flex items-center justify-between gap-4 rounded-xl border border-white/5 bg-white/[0.02] px-4 py-3 transition hover:bg-white/[0.04]"
              >
      ...
      <motion.div variants={stagger} className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          ...
          {insights.activeIncidents.length === 0 ? (
            ...
            <div className="space-y-2">
              {insights.activeIncidents.slice(0, 3).map((inc) => (
                <Link key={inc.id} to={`/monitors/${inc.monitor.id}`} className="flex items-center justify-between gap-3 rounded-xl border border-white/5 bg-white/[0.02] px-4 py-3 hover:bg-white/[0.04] transition">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-white truncate">{inc.monitor.name}</p>
                    <p className="text-[10px] text-slate-500 mt-0.5 truncate">{inc.message}</p>
                  </div>
        ...
        <motion.div variants={fadeUp} className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-xs font-bold uppercase tracking-[0.15em] text-slate-500">Quick Stats</h3>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-600">Alerts 24h</p>
              <p className="mt-1 text-xl font-extrabold text-white">{insights.alertsLast24h}</p>
            </div>
            <div className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-600">Critical</p>
              <p className="mt-1 text-xl font-extrabold text-white">{insights.criticalIncidentCount}</p>
            </div>
            <div className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-600">Impact Coverage</p>
              <p className="mt-1 text-xl font-extrabold text-white">{insights.impactCoverage}%</p>
            </div>
            <div className="rounded-xl border border-white/5 bg-white/[0.02] p-3">
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-600">Watch time</p>
              <p className="mt-1 text-xl font-extrabold text-white">{insights.activeWatchMinutes}m</p>
            </div>
          </div>
        </motion.div>
      </motion.div>
