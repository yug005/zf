...
export function NotificationBell() {
  ...
  const notificationIcon = (type: string) => {
    if (type === 'TRIAL_EXPIRING') return <Clock3 className="h-4 w-4 text-amber-600" />;
    if (type === 'TRIAL_EXPIRED') return <AlertTriangle className="h-4 w-4 text-red-600" />;
    if (type === 'MONITOR_LIMIT_REACHED') return <AlertTriangle className="h-4 w-4 text-amber-600" />;
    if (type === 'SUBSCRIPTION_ACTIVATED') return <CircleCheck className="h-4 w-4 text-emerald-600" />;
    if (type === 'SUBSCRIPTION_CANCELLED') return <CreditCard className="h-4 w-4 text-orange-600" />;
    if (type === 'PAYMENT_FAILED' || type === 'PAYMENT_FAILED_FINAL') {
          ...
          <div className="max-h-80 overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`flex items-start gap-3 border-b border-gray-50 px-4 py-3 last:border-0 ${notificationTone(notification.type)} ${!notification.read ? 'font-medium' : ''}`}
                >
                  <span className="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full bg-white/80">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[10px] text-gray-400">{formatTime(notification.createdAt)}</p>
                  </div>
                </div>
              ))
            )}
